...
import { Loader2, ShieldCheck, Wifi, Mail, Lock, Router, WifiOff } from 'lucide-react';
...
function LoginForm() {
    ...
    // Grab the MikroTik parameters from the captive portal redirect
    const mac = searchParams.get('mac') || '';
    const ip = searchParams.get('ip') || '';

    // Secret Admin Backdoor: Add ?admin=true to the URL if you are at home!
    ...
    // ==========================================
    // THE WI-FI LOCKOUT SCREEN
    // ==========================================
    // If the router didn't pass a MAC address, and you aren't using the admin backdoor, block the UI.
        ...
        return (
            <div className="w-full max-w-md bg-zinc-900/60 border border-rose-500/30 rounded-3xl shadow-2xl p-8 backdrop-blur-xl z-10 text-center animate-in zoom-in-95 duration-500">
                ...
                <p className="text-sm text-zinc-400 mt-3 leading-relaxed">
                    You must be physically connected to the <strong>Library Local Wi-Fi</strong> network to access this portal.
                </p>
                <p className="text-xs text-zinc-500 mt-6 pt-6 border-t border-zinc-800/80">
                    If you are connected to the library but seeing this error, please disconnect and reconnect to the Wi-Fi to trigger the gateway.
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="w-full max-w-md bg-zinc-900/60 border border-zinc-800/80 rounded-3xl shadow-2xl p-8 backdrop-blur-xl z-10 animate-in fade-in zoom-in-95 duration-500">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
                {/* HIDDEN INPUTS: These send the hardware data to auth.ts */}
            ...
            <div className="mt-8 text-center border-t border-zinc-800/80 pt-6">
                <p className="text-xs text-zinc-500">
                    Need an account? See the library administrator at the front desk.
                </p>
            </div>
        </div>
    );
